function [P_t5, T_t5cool, M5, A_Out_Comp, L_turbina] = Turbina()
...
T_t5cool = ((1 - eps_cool +f) * cp_GC * T_t4 + eps_cool * cp_a * T_tiii - Pt/ m_a) / ((1 - eps_cool) * cp_GC + eps_cool * cp_a);
taut = T_t5cool / T_t4;
pi_t = taut ^ (g_GC / ((g_GC - 1) * e_t));

T5_cool = T_t5cool - (0.5 * Cz_ii^2) / cp_GC;
M5 = Cz_ii / sqrt(g_GC * R_GC * T5_cool);
P_t5 = pi_t * P_t4;
